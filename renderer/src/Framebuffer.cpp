...
void Framebuffer::createColorAttachment(unsigned int format)
{
...
void Framebuffer::createMultisampleColorAttachment(unsigned int samples, GLenum internalFormat)
{
	...
	bind();
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + textures.size() - 1, GL_TEXTURE_2D_MULTISAMPLE, texture, 0);
	unbind();
}
